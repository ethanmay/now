...
function get_news_by_network( $network ) {
	...
	switch( $network ) {
		...
		case 'all':
			$news['abc'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 4 LIMIT 30" );
			$news['aljazeera'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 5 LIMIT 30" );
			$news['bbc'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 6 LIMIT 30" );
			$news['cnn'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 7 LIMIT 30" );
			$news['fox'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 8 LIMIT 30" );
			$news['google'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 9 LIMIT 30" );
			$news['frontline'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 10 LIMIT 30" );
			$news['newrepublic'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 11 LIMIT 30" );
			$news['huffpost'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 12 LIMIT 30" );
			$news['nytimes'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 13 LIMIT 30" );
			$news['npr'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 14 LIMIT 30" );
			// $news['reddit'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 15 LIMIT 30" );
			// $news['reddit_top'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 16 LIMIT 30" );
			$news['wired'] = $db->get_results( "SELECT content FROM headlines WHERE network_id = 17 LIMIT 30" );
			break;
		case 'abc':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 4 LIMIT 30" );
			break;
		case 'aljazeera':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 5 LIMIT 30" );
			break;
		case 'bbc':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 6 LIMIT 30" );
			break;
		case 'cnn':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 7 LIMIT 30" );
			break;
		case 'fox':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 8 LIMIT 30" );
			break;
		case 'google':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 9 LIMIT 30" );
			break;
		case 'frontline':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 10 LIMIT 30" );
			break;
		case 'newrepublic':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 11 LIMIT 30" );
			break;
		case 'huffpost':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 12 LIMIT 30" );
			break;
		case 'nytimes':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 13 LIMIT 30" );
			break;
		case 'npr':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 14 LIMIT 30" );
			break;
		// case 'reddit':
		// 	$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 15 LIMIT 30" );
		// 	break;
		// case 'reddit_top':
		// 	$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 16 LIMIT 30" );
		// 	break;
		case 'wired':
			$news = $db->get_results( "SELECT content FROM headlines WHERE network_id = 17 LIMIT 30" );
			break;
		default:
			error_log( "my bad" );
			break;
